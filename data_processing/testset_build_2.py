...
def test_set_build(simple, normal, context_test, no_context_test, testset1, testset2, output_test, output_test2):
    ...
    for i in range(len(simple)):
        ...
        for t in range(0, len(s_sent)-3, 2):
            temp_array = s_sent[:t+2] + ["[MASK]"] * (len(s_sent)-t-3) + [s_sent[-1]]
            label = " ".join(s_sent[t+2:t+2+min(2,len(s_sent)-t-3)])
...
def main():
    if len(sys.argv) != 9:
        print("Need 9 arguments: [usage] python3 testset_build_2.py simple.txt normal.txt dev_context_2.txt dev_no_context_2.txt test_context_2.txt test_no_context_2.txt dev_labels_2.txt test_labels_2.txt")
    else:
