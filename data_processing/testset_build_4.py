...
def test_set_build(simple, normal, context_test, no_context_test, testset1, testset2, output_test, output_test2):
    ...
    for i in range(len(simple)):
        ...
        for t in range(0, len(s_sent)-5, 4):
            # fix the algorithm here to take care of predicting next 4 words
            temp_array = s_sent[:t+4] + ["[MASK]"] * (len(s_sent)-t-5) + [s_sent[-1]]
            label = " ".join(s_sent[t+4:t+4+min(4,len(s_sent)-t-5)])
...
def main():
    if len(sys.argv) != 9:
        print("Need 9 arguments: [usage] python3 testset_build_4.py simple.txt normal.txt dev_context_4.txt dev_no_context_4.txt test_context_4.txt test_no_context_4.txt dev_labels_4.txt test_labels_4.txt")
    else:
